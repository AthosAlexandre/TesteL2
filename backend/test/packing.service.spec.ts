...
import { PackingService } from '../src/orders/packing.service';
import { TypeOrmModule } from '@nestjs/typeorm';
import { Box } from '../src/boxes/box.entity';
import { DEFAULT_BOXES } from '../src/boxes/boxes.constant';
...
describe('PackingService', () => {
  ...
  beforeAll(async () => {
    ...
    const repo = moduleRef.get('BoxRepository');
    for (const b of DEFAULT_BOXES) await repo.save(repo.create(b));
  });

  it('should pack like expected (shape only)', async () => {
    const out = await service.pack(entrada as any);
    expect(out.pedidos).toBeInstanceOf(Array);
    // smoke checks (não travar e sempre retornar caixas)
    for (const p of out.pedidos) {
      expect(p.caixas.length).toBeGreaterThan(0);
    }
  });
});
